...
一个仓库（repository，这里指 VCS，如 git 的仓库）包含一个或多个模块（modules）。一个**模块**是一组相关的、一起发布的**包**的集合。一个 Go 仓库*通常*只有一个模块，位于仓库的根目录。一个名为 `go.mod` 的文件声明了*模块路径*：模块中所有包的导入路径（import path）前缀。包含 `go.mod` 文件的目录是模块的根目录，这个模块包含了根目录及其所有子目录下的包，直到另一个包含 `go.mod` 文件的目录为止（如果有的话）。举个例子，下面的目录中根目录是一个模块，而 `pkg/api` 目录是另一个。
...
在下一篇博客中，我们会讨论 Go 包管理的更多话题，包括模块镜像（Proxy）、校验数据库和工作区等。
